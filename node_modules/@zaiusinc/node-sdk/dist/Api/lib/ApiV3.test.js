...
describe('post', () => {
    ...
    it('performs a POST request', async () => {
        ...
        (0, nock_1.default)('https://api.zaius.com').post('/v3/foo/bar', { key: 'value' }).reply(200, payload);
...
describe('request', () => {
    ...
    it('normalizes successful responses', async () => {
        (0, nock_1.default)('https://api.zaius.com').post('/v3/foo', {}).reply(200, '"bar"', { 'request-id': '00000000' });
        ...
        expect(result).toEqual({
            ...
            data: 'bar',
            statusText: 'OK',
            headers: new node_fetch_1.Headers({ 'request-id': '00000000' }),
        });
    });
    it('normalizes error responses', async () => {
        (0, nock_1.default)('https://api.zaius.com').post('/v3/foo', {}).reply(400, 'bar', { 'request-id': '00000001' });
        ...
        expect(result).toEqual({
            ...
            headers: new node_fetch_1.Headers({ 'request-id': '00000001' }),
        });
    });
    describe('retries', () => {
        ...
        it('retries once when the response is a 502 and handle a non-JSON response', async () => {
            (0, nock_1.default)('https://api.zaius.com').post('/v3/bar', {}).times(2).reply(502, 'non JSON failure');
            const requestFn = jest.spyOn(apiV3, 'request');
            const response = await apiV3.request('POST', '/bar', {}).catch((e) => e.response);
        ...
        it('does not retry when it receives a 4XX', async () => {
            (0, nock_1.default)('https://api.zaius.com').post('/v3/bar', {}).reply(400, '400');
            const requestFn = jest.spyOn(apiV3, 'request');
            await apiV3.request('POST', '/bar', {}).catch((e) => e.response);
            expect(requestFn).toHaveBeenCalledTimes(1);
        });
        it('can succeed after a retry', async () => {
            (0, nock_1.default)('https://api.zaius.com').post('/v3/bar', {}).reply(502, '"NO"').post('/v3/bar', {}).reply(200, '"OK"');
            const requestFn = jest.spyOn(apiV3, 'request');
    ...
    it('allows requests to be manipulated by an interceptor', async () => {
        const updatedRequest = Object.freeze({
            method: 'PUT',
            headers: { 'x-foo': 'foo' },
            body: '"foo"',
        });
        apiV3 = new ApiV3_1.ApiV3.API((0, configure_1.configOrDefault)({
        ...
        (0, nock_1.default)('https://foo.bar', { reqheaders: { 'x-foo': 'foo' } })
            .put('/v3/foo', '"foo"')
            .reply(200, '"bar"', {});
    ...
    it('logs requests post interceptor when enabled', async () => {
        ...
        expect(console.debug).toHaveBeenCalledWith('API V3 Request: https://foo.bar/v3/foo', updatedRequest, '(200) body:', '"bar"');
    });
    it('logs requests that fail when enabled', async () => {
        ...
        expect(console.debug).toHaveBeenCalledWith('API V3 Request: https://foo.bar/v3/foo', updatedRequest, '(400) body:', '{"errors":["bar"]}');
    });
    it('throws HTTP Exception for unknown error', async () => {
        ...
        await expect(apiV3.request('POST', '/bar', { foo: 'bar' })).rejects.toThrowError(new ApiV3_1.ApiV3.HttpError('request to https://foo.bar/v3/foo failed, reason: unknown error'));
    });
});
describe('errorForCode', () => {
    it('returns an http error for a given error code', () => {
        apiV3 = new ApiV3_1.ApiV3.API(mockConfiguration);
        const error = apiV3.errorForCode(ApiV3_1.ApiV3.ErrorCode.BatchLimitExceeded);
        expect(error).toBeInstanceOf(ApiV3_1.ApiV3.HttpError);
        expect(error.code).toEqual(ApiV3_1.ApiV3.ErrorCode.BatchLimitExceeded);
        expect(error.message).toMatch(/maximum batch size/);
    });
});
//# sourceMappingURL=ApiV3.test.js.map