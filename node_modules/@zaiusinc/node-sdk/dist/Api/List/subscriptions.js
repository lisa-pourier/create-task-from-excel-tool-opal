 ...
 */
async function subscribe(apiV3, listId, identifiers) {
    if (!Array.isArray(identifiers)) {
        identifiers = [identifiers];
    }
    return updateSubscriptions(apiV3, listId, identifiers.map((id) => ({ ...id, subscribed: true })));
}
/**
 ...
 * @throws {HttpError} if it receives a non-2XX result or if the batch size is > BATCH_LIMIT
 */
async function unsubscribe(apiV3, listId, identifiers) {
    if (!Array.isArray(identifiers)) {
        identifiers = [identifiers];
    }
    return updateSubscriptions(apiV3, listId, identifiers.map((id) => ({ ...id, subscribed: false })));
}
/**
 ...
 * @throws {HttpError} if it receives a non-2XX result or if the batch size is > BATCH_LIMIT
 */
async function updateSubscriptions(apiV3, listId, updates) {
    if (updates.length > ApiV3_1.ApiV3.BATCH_LIMIT) {
        throw apiV3.errorForCode(ApiV3_1.ApiV3.ErrorCode.BatchLimitExceeded);
    }
    return await apiV3.post('/lists/subscriptions', updates.map((update) => ({ list_id: listId, ...update })));
}
